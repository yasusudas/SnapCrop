...
function drawMagnifier(magCanvas, srcCanvas, imgX, imgY) {
  ...
  ctx.lineTo(MAG_HALF * 2, MAG_HALF);
  ctx.stroke();
...
function findBoundaryFromBottomAtX(imageData, x, target, tolerance) {
  for (let y = imageData.height - 1; y >= 0; y -= 1) {
    if (matchesToleranceAt(imageData, x, y, target, tolerance)) {
...
export default function ColorBasedPixelCropper() {
  ...
  async function handlePrepareActivePng() {
    ...
    try {
      ...
      setStatus("PNGダウンロードリンクを生成しました。リンクをクリックして保存してください。");
    } catch (error) {
  ...
  async function handlePrepareZip() {
    ...
    try {
      ...
      setZipUrl(url);
      setZipName(name);
      setStatus(`ZIPダウンロードリンクを生成しました (${processedItems.length} 枚)。リンクをクリックして保存してください。`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <motion.div
          ...
          className="space-y-2"
        >
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 md:text-4xl">画像複数キュー一括切り抜き</h1>
          <p className="text-sm leading-6 text-slate-600 md:text-base">
            複数画像をキューとして読み込み、1枚目で選択した基準色と設定を全画像へ一括適用します。
            画像処理ロジックはそのまま維持し、UXだけを複数画像対応にしています。
          </p>
        </motion.div>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-3">
                ...
                >
                  ...
                  <span>{isLoading ? "読み込み中…" : "jpg / png を複数選択"}</span>
                </label>
                <input
                  ref={fileInputRef}
                  id="file"
              ...
              <div className="grid gap-3 rounded-2xl bg-slate-100 p-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-700">キュー枚数</span>
                  ...
                  <div className="min-w-0 flex-1 text-sm text-slate-600">
                    {hoverColor ? (
                      <div className="space-y-1 italic text-slate-400">
                        <div>
                          R {hoverColor.r} / G {hoverColor.g} / B {hoverColor.b} / A {hoverColor.a}
                        </div>
                      ...
                      <div className="space-y-1">
                        <div>
                          R {selectedColor.r} / G {selectedColor.g} / B {selectedColor.b} / A {selectedColor.a}
                        </div>
                        <div>
                          座標: ({selectedColor.x}, {selectedColor.y})
                        </div>
                      </div>
                    ) : (
                      <div>1枚目の画像をクリックして基準色を選択</div>
                    )}
                  </div>
                </div>
                <Button
              ...
              <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
                ...
                {[
                  ...
                  <div key={key} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-3">
                    <Label htmlFor={key} className="text-sm text-slate-700">
                      {label}
                    </Label>
                    <Switch
                      id={key}
                      checked={directions[key]}
                      onCheckedChange={(checked) => setDirections((prev) => ({ ...prev, [key]: checked }))}
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Button onClick={handleCropAll} className="rounded-2xl" disabled={!queue.length || !selectedColor || isCroppingAll}>
                ...
                <CardContent>
                  ...
                  {activeItem?.croppedData && (
                    <div className="mt-4 space-y-2">
                      <Button variant="secondary" className="w-full rounded-2xl" onClick={handlePrepareActivePng}>
                        PNGダウンロードリンクを生成
                      </Button>
                      {activePngUrl && (
                        <a
                  ...
                  {queue.map((item, index) => (
                    <button
                      ...
                      className={`block w-full rounded-2xl border p-3 text-left transition ${
                        item.id === activeId ? "border-slate-900 bg-slate-50" : "border-slate-200 bg-white"
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        <div className="h-20 w-16 flex-shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
                          {item.previewUrl && <img src={item.previewUrl} alt={item.name} className="h-full w-full object-contain" />}
                        </div>
